...
void handle_client_message(int ser, const unsigned char server_pk[PK_LEN],
    ...
    if (strncmp((const char*)msg, "/send ", 6) == 0) {
                ...
                int idx = -1;
                int consumed = 0;
                if (sscanf((const char*)msg+6, "%d%n", &idx, &consumed) >= 1) {
                    const char *msg2 = (const char*)msg + 6 + consumed;
                    while (*msg2 == ' ') msg2++;
                    if (idx >= 0 && idx < client_count && *msg2) {
                        if (server_send_to_client(ser, server_pk, idx, (const unsigned char*)msg2, strlen(msg2)) == 0) {
